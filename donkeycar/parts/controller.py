...
class RC7ChanJoystick(Joystick):
    # An interface to a physical joystick available at /dev/input/js0
    def __init__(self, *args, **kwargs):
        super(RC7ChanJoystick, self).__init__(*args, **kwargs)
...
class RC7ChanJoystickController(JoystickController):
    # A Controller object that maps inputs to actions
    def __init__(self, *args, **kwargs):
        super(RC7ChanJoystickController, self).__init__(*args, **kwargs)

    def init_js(self):
        # attempt to init joystick
        try:
            self.js = RC7ChanJoystick(self.dev_fn)
            self.js.init()
        except FileNotFoundError:
            print(self.dev_fn, "not found.")
            self.js = None
    ...
    def init_trigger_maps(self):
        ...
        self.button_down_trigger_map = {
            ...
            'end': self.toggle_mode,
            'dr': self.emergency_stop,
            'ch3': self.toggle_manual_recording,
            # 'ch3trim' : self.toggle_constant_throttle,
        }
...
def get_js_controller(cfg):
    ...
    elif cfg.CONTROLLER_TYPE == "rc7":
        cont_class = RC7ChanJoystickController
